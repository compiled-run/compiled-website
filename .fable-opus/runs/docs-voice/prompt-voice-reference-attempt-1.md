Fable-Opus-Unit: docs-voice/voice-reference
Fable-Opus-Parallel: yes

## Goal

Retune the prose voice of one Markless docs page. A previous pass already gave it bite-size rhythm; this pass adjusts REGISTER only. Most paragraphs need no change; on a reference page, terse API listings and tables are already the right voice and stay untouched. Rewrite only the connecting prose where the register is off.
...
Preserve exactly: all code blocks, tables, imports, component tags and their non-text props, links and targets, heading meaning, diagnostic codes, and every fact, number, warning, caveat, and finding reference. Component text props stay single-line valid JSX with no raw double quotes and get the same voice rules. Keep hard wraps near 100 characters.
...
- Do not add or delete sections, restructure the page, or change facts. Why: this is a voice pass, and factual drift is worse than an off-register sentence.
