...
<Callout sticker="spring" sprite="cross" title="Why the panels on this page are files rather than boxes" text="Every other page here hands you something to click. This one does not, and the honest reason is that a component using @if does not compile on the framework build this site is pinned to: the compiler stops making progress and the build never finishes. Both files below are real, and both are the specification's own example, so they are what you will run in your own project. The panels come back to this page the day the build accepts them. NOTES.md finding 23 in the site repository has the reproduction." />
...
<Callout sprite="speech-bubble" title="Will anything warn me?" text="Not today. The specification says the compiler should be able to explain this when local state appears inside a conditional branch, in these words: State declared inside @if is disposed when the branch is removed. Move it above the @if if it should persist. That message is a plan, not a thing you will see: the published 0.3.1 compiler carries no such diagnostic, and we checked by searching its build output for the word disposed and finding none. So this rule is one to remember rather than one you will be reminded of." />
